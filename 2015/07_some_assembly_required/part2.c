...
void reset_signals(struct Circuit *circuit){
	int i;

	for(i = 0; i < circuit->size; i++){
		circuit->wires[i]->solved = false;
	}
}
...
int main(){
	...
	command->gate = ASSIGN;
	sprintf(command->lhs, "%d", evaluate_wire(circuit, "a"));
	strcpy(command->out, "b");

	struct Wire *w = get_wire(circuit, "b");
	free(w->command);
	w->command = command;

	reset_signals(circuit);
