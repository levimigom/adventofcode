...
void mark_antinode(int rows, int cols, bool **is_antinode, int row, int col, int row_diff, int col_diff){
	int new_row = row;
	int new_col = col;

	while(is_within_bounds(rows, cols, new_row, new_col)){
		is_antinode[new_row][new_col] = true;
	...
	for(i = 0; i <= row; i++){
		for(j = 0; j < cols && (i < row || j < col); j++){
			if(grid[row][col] == grid[i][j]){
				...
				mark_antinode(rows, cols, is_antinode, i, j, - row_diff, - col_diff);
				mark_antinode(rows, cols, is_antinode, row, col, row_diff, col_diff);
			}
		}
	}
}
